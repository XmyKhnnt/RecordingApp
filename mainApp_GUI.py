...
from PyQt5.QtCore import QRegExp, QSize, Qt, QThread, QTimer, pyqtSignal
from PyQt5.QtGui import (QColor, QFont, QIcon, QKeyEvent, QKeySequence,
                         QPixmap, QRegExpValidator, QTextOption)
...
# ------------------------------------------------------------
        ...
        self.recording_gap_text_input.setMaximumHeight(25)
        self.recording_gap_text_input.setMaximumWidth(100)
        self.recording_gap_text_input.setPlaceholderText("Seconds")
...
'''
        ...
        self.recording_gap_text_input.setStyleSheet("""
            border: none;   
        """)
        self.recording_gap_text_input.setGraphicsEffect(word_box_shadow)
    ...
    def add_another_frame_to_scroll_area(self):
            ...
            sequence = self.count_number_of_frame() + 1
    ...
    def addFrame(self):
        try:
            nltk_text = self.nltkSentenceSeparator()
            if len(nltk_text) > 1:

                for add_frame in range(1,len(nltk_text)):
                    setFrameText = nltk_text[add_frame]
            ...
            self.new_frame.text_place_holder.setText(nltk_text[0])  
        except:
            pass

    def nltkSentenceSeparator(self):
        text = self.new_frame.text_place_holder.toPlainText()
        try:
